...
## End default shell integration. Custom stuff below
...
#    iterm2_set_user_var asdfRubyVersion $(asdf current ruby | sed 's/ .*$//')
    ...
    while IFS= read -r line; do
        ...
        if [[ ! -z "$asdf_available" ]]; then
            asdf_available="$asdf_available,$name"
        else
            asdf_available="$name"
        fi
        iterm2_set_user_var "asdf_$name" "$version"
    done < <(asdf current 2>&1 > /dev/null)
    iterm2_set_user_var asdf_available "$asdf_available"
}
